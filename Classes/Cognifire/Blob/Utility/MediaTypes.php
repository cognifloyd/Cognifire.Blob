...
These are here but commented out until someone says we need them
(Package Builder will take a performance hit w/ all of them):
...
- wikipedia says that actionscript is application/ecmascript

I might have to update TYPO3.Flow/Scripts/updatemediatypes.php to include this custom
list of types in there so that we can continue to use that script and get updates w/o
requiring that we turn try to get apache to update their list just so that we can
easily search for files by type here.
*/
...
//		'phps' => 'application/x-php',
		...
		'ts' => 'application/x-typoscript',
		'ts2' => 'application/x-typoscript',
...
//		'application/x-php' => array('php','php3','php4','php5','phps','phtml','inc'),
		'application/x-php' => array('php'),
		...
		'application/x-actionscript' => array('as'),
		...
		'application/x-coffeescript' => array('coffee'),
		...
		'application/x-less-css' => array('less'),
		...
		'application/x-python' => array('py'),
		'application/x-rar-compressed' => array('rar'),
		'application/x-research-info-systems' => array('ris'),
		'application/x-ruby' => array('rb'),
		'application/x-sass-css' => array('sass','scss'),
		...
		'application/x-typoscript' => array('ts2','ts'),
		...
		'application/x-yaml' => array('yaml','yml'),
